...
function covercutter(rc) {
    /*从内容中去除所有<ifcover>的部分*/
    var rst = rc;
...
function readsave() {
    if (confirm('真的要读取草稿吗？\n这会覆盖你目前的内容')) {
        ...
        SC('content').value = localStorage.ocontent;
    }
}
...
function delpost(id) {
    ...
    if (tj['postindex'][id]) {
        ...
        new Promise(function(res, rej) {
            ...
            return new Promise(function(res, rej) {
                blog.cr(window.accesstoken, window.githubrepo, 'index.html', 'Delete post', indexpage, {
                    /*渲染首页*/
                    success: function(m) {
                        loadhide();
                        notice('删除成功');
