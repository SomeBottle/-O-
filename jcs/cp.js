...
var editpost = 'none'; /*编辑的文章*/
if (!window.tjson) {
	loadshow();
	$.aj('./template/template.json', {}, { /*获得模板内容*/
		success: function(k) {
			window.tjson = JSON.parse(k);
			if (!window.mainjson) {
...
function tagarchive() { /*生成tag和归档页面*/
	...
	var rendert5 = B.r(rendert4, '{[type]}', 'tags.html'); /*Archives*/
	...
	var rendera5 = B.r(rendera4, '{[type]}', 'archives.html');
	if (confirm('确定要生成标签和归档页面？')) {
		loadshow();
		blog.cr(window.githubuser, window.githubrepo, 'tag.html', 'Generate Tag Page', rendert5, {
			success: function(m) {
				blog.cr(window.githubuser, window.githubrepo, 'archive.html', 'Generate Archive Page', rendera5, {
					success: function(m) {
						notice('Successful:D');
						loadhide();
					},
...
function indexrenderer(js) { /*首页渲染者*/
	var tj = js;
	...
	var renderi4 = B.r(renderi3, '{[type]}', 'postlist.html');
...
function edit() {
	...
	if (isNaN(date)) {
		ifpage = true;
		if (window.tjson.alltp.indexOf(date + '.html') !== -1) {
			notice('链接名与模板重复！');
			notice('请更换');
			return false; /*跳出函数*/
		}
	...
	if (confirm('真的要发布嘛？')) {
		...
		var render7 = B.r(render6, '{[type]}', ptype + '.html');
...
function searchrender(v) {
	...
	for (var i in pt) {
		...
		} else if (cc.indexOf(v) !== -1) {
			rendertp += "<p class='scitemp'><a class='scitema' href='javascript:void(0);' onclick='postopen(" + i + ")'>" + tt + "</a></p>";
		} else if (dd.indexOf(v) !== -1) {
			rendertp += "<p class='scitemp'><a class='scitema' href='javascript:void(0);' onclick='postopen(" + i + ")'>" + tt + "</a></p>";
		} else if (tg.indexOf(v) !== -1) {
			rendertp += "<p class='scitemp'><a class='scitema' href='javascript:void(0);' onclick='postopen(" + i + ")'>" + tt + "</a></p>";
		}
	}
	if (rendertp == '') {
