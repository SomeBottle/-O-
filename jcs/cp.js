...
var editpost = 'none';
var tpjs=JSON.parse(window.tjson);
/*编辑的文章*/
if (!window.mainjson) {
	loadshow();
...
function tagarchive() {
    ...
    var rendert5 = B.r(rendert4, '{[type]}', pageh1);
    /*Archives*/
...
function indexrenderer(js) {
    ...
    var renderi3 = B.r(renderi2, '{[keywords]}', tj['name']);
    /*站点名字加入keywords*/
    var renderi4 = B.r(renderi3, '{[type]}', pageh);
    var renderi5 = B.r(renderi4, '{[sitename]}', tj['name']);
    /*设定title*/
    var renderi6 = B.r(renderi5, '{[content]}', listrender);
    /*注入灵魂*/
    return renderi6;
}
function tempsave() {
...
function preview() {
    ...
    SC('sbr').innerHTML = '<a class=\'closebtn\' href=\'javasccript:void(0);\' onclick=\'sbrclose()\'>×<\/a><h2>Preview:<\/h2><style>img{max-width:100%;}</style>' + md.makeHtml(content);
}
function edit() {
    ...
    if (confirm('真的要发布嘛？')) {
        ...
        var render5 = B.r(render4, '{[description]}', intro.replace(/<\/?.+?>/g, ""));
        /*去除html标签作为description*/
        var render6 = B.r(render5, '{[keywords]}', tag + ',' + tj['name']);
        /*站点名字加入keywords*/
        var render7 = B.r(render6, '{[type]}', pageh);
        var render8 = B.r(render7, '{[sitename]}', tj['name']);
        /*设定title*/
        /*RenderFinish*/
        if (tj[0] == 'initialized') {
            /*删除默认内容*/
        ...
        var render9 = B.r(render8, '{[pid]}', nownum);
        /*设定pid*/
        var pagelink = '';
        /*指定编辑的页面的link(不带.html)*/
        if (ifpage) {
            /*如果是页面直接获取当前时间*/
            pagelink = date;
            date = getdate();
        }
        tj['dateindex']['post' + nownum] = date + nownum.toString();
        /*利用date进行排序*/
        var datearray = new Array();
        for (var dts in tj['dateindex']) {
            datearray.push(Array(dts, parseInt(tj['dateindex'][dts])));
            /*转二维数组*/
        }
        tj['dateindex'] = new Object();
        ...
        var recentlink = '';
        /*如果是页面，储存修改之前的页面名*/
        if (editpost !== 'none' && !ifpage && tj['postindex'][nownum]['link']) {
            /*不能将页面转为文章*/
            notice('你正在编辑页面');
            notice('不能转为文章');
            return false;
        ...
        if (ifpage) {
            tj['postindex'][nownum]['link'] = pagelink;
            /*储存pagelink*/
        }
        tj['postindex'][nownum]['intro'] = Base64.encode(intro);
        /*去除html标签的Intro*/
        ...
        var indexpage = indexrenderer(tj);
        /*渲染首页*/
                ...
                blog.cr(window.githubuser, window.githubrepo, 'main.json', commit, JSON.stringify(tj), {
                    success: function(m) {
                        blog.cr(window.githubuser, window.githubrepo, 'index.html', commit, indexpage, {
                            success: function(m) {
                                if (recentlink !== '' && recentlink !== pagelink) {
                                    /*更新页面时如果更改链接,删除先前存在的页面*/
        ...
        var tm = setInterval(function() {
            if (window.gstate <= 0) {
                ...
                /*渲染最新文章列表*/
                if (editpost !== 'none') {
                    window.htmls['post-' + editpost + '.html'] = Base64.encode(render9);
                    /*更新变量储存的post*/
                } else {
                    postopen(nownum);
                    /*发布文章后跳转到编辑模式*/
            ...
            success: function(m) {
                ...
                blog.cr(window.githubuser, window.githubrepo, 'main.json', 'Delete post', JSON.stringify(tj), {
                    success: function(m) {
                        ...
                        renderlist();
                        /*渲染最新文章列表*/
                        blog.cr(window.githubuser, window.githubrepo, 'index.html', 'Delete post', indexpage, {
                            /*渲染首页*/
                            success: function(m) {
                                loadhide();
                                notice('删除成功');
...
function postopen(id) {
    ...
    this.loadpost = function(ct) {
        ...
        var content = (B.gt('<!--[PostContent]-->', '<!--[PostContentEnd]-->', ht)).trim();
        /*去除内容首尾的空格*/
    ...
    if (!window.htmls[filename]) {
        ...
        blog.getfile(window.githubuser, window.githubrepo, filename, true, {
            success: function(fi) {
                ...
                window.htmls[filename] = fi.content;
                /*已经加载过的文章先存着*/
            },
            failed: function(m) {
                loadhide();
...
function searchrender(v) {
    ...
    rendertp = "<a class='closebtn' href='javasccript:void(0);' onclick='sbrclose()'>×</a>" + rendertp;
    /*关闭按钮*/
